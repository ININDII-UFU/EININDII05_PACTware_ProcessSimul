...
COMMANDS: Dict[str, Dict[str, Any]] = {
    ...
    "BD": {"resp": ["error_code"]},
    "CC": {"resp": ["error_code"]},
    "85": {"resp": [{"MAP": {"KEY": "$SEL2", "TABLE": {}, "DEFAULT": "error_code"}}]},
    "A0": {"resp": [{"MAP": {"KEY": "$SEL2", "TABLE": {}, "DEFAULT": "error_code"}}]},
}
...
class HrtTransmitter:
    ...
    def response(self, hrt_frame_read: HrtFrame) -> HrtFrame:
        ...
        if not spec:
            self._hrt_frame_write.body = self._get("error_code", "0000")
            return self._hrt_frame_write
        ...
        # resposta
        self._hrt_frame_write.body = self._eval_list(spec.resp, ctx) if spec.resp else self._get("error_code", "0000")
