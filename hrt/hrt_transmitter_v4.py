...
class HrtTransmitter:
    ...
    def _get(self, row_key: str, default_hex: str | None = None) -> str:
        """Retorna o valor HEX (par) de uma variável no DB (ReactVar), em modo estrito.

        - Se a variável não existir (mesmo após aliases), lança KeyError (a menos que default_hex seja fornecido).
        - Se a tradução não resultar em HEX, lança ValueError (a menos que default_hex seja fornecido).

        Isso evita 'forçar resposta' com valor errado.
        """
        aliases: Dict[str, str] = {
            "write_protect": "write_protect_code",
        ...
        if not self._has(key):
            key = aliases.get(row_key, row_key)

        if not self._has(key):
            if default_hex is not None:
                return default_hex
            raise KeyError(f"DB missing row '{row_key}' (resolved '{key}')")

        raw = self.db.translate(key)  # type: ignore[attr-defined]
        if raw is None:
            if default_hex is not None:
                return default_hex
            raise ValueError(f"DB.translate('{key}') returned None")
        ...
        if not _is_hex(v):
            if default_hex is not None:
                return default_hex
            raise ValueError(f"DB.translate('{key}') returned non-HEX: {raw!r}")
    ...
    def _err_body(self, response_code: str = "02") -> str:
        """Corpo mínimo de erro: error_code(2B)+response_code(1B)+device_status(1B)+comm_status(1B)."""
        def safe(key: str, default_hex: str) -> str:
            try:
        ...
        return (
            safe("error_code", "0000")
            + response_code.upper().zfill(2)
            + safe("device_status", "40")
            + safe("comm_status", "00")
        )

    def _make_reply(self, command: str, body: str) -> HrtFrame:
        self._prime_header()
        self._hrt_frame_write.command = command.upper()
    ...
    def response(self, hrt_frame_read: HrtFrame) -> HrtFrame:
        command = (hrt_frame_read.command or "").upper()
        spec = COMMANDS.get(command)

        if spec is None:
            # Comando não mapeado => erro explícito
            body = self._err_body(response_code="02")
            return self._make_reply(command, body)

        try:
            # side-effects antes (write)
            for op in spec.get("write", []) or []:
                self._eval_token(op, body=hrt_frame_read.body or "")

            # corpo de resposta
            resp_seq = spec.get("resp", []) or []
            body = self._eval_sequence(resp_seq, body=hrt_frame_read.body or "")

            # side-effects depois (after)
            for op in spec.get("after", []) or []:
                self._eval_token(op, body=hrt_frame_read.body or "")

        except Exception as e:
            # Se faltar variável / tradução inválida / etc, devolve erro.
            try:
                self.log.append(f"[ERR] cmd={command} {type(e).__name__}: {e}")
            except Exception:
                pass
            body = self._err_body(response_code="02")
