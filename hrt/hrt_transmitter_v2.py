...
class HrtTransmitter:
    ...
    def request(self, hrt_frame_read: HrtFrame) -> Union[HrtFrame, str]:
        ...
        # Comandos estendidos identificados no 0301.TBL como "write_*".
        # Para estes, quando este transmissor estiver atuando como master,
        # usamos um payload previamente armazenado (cmd_<CMD>_payload).
        write_ext_cmds = {"82", "83", "87", "A8"}
        ...
        empty_req = {
            ...
            # comandos estendidos “read” (mantém compatibilidade)
            "84", "88", "8A", "8C", "98", "A2", "A4", "A6", "AD", "B9", "BB", "C6", "DF",
        }

        if cmd in empty_req:
            self._hrt_frame_write.body = ""

        elif cmd in write_ext_cmds:
            # Payload previamente armazenado por uma escrita recebida (response).
            # Se ainda não existe, mantém vazio (não falha).
            self._hrt_frame_write.body = g(f"cmd_{cmd}_payload", "")
    ...
    def response(self, hrt_frame_read: HrtFrame) -> HrtFrame:
        ...
        # Comandos estendidos (0301.TBL) que são "write_*".
        # Guardamos o payload recebido para facilitar replays/diagnóstico.
        write_ext_cmds = {"82", "83", "87", "A8"}
        if cmd in write_ext_cmds and (hrt_frame_read.body is not None):
            s(f"cmd_{cmd}_payload", hrt_frame_read.body)
        ...
        # --------------------------- Comandos estendidos (mantidos como estavam) ---------------------------
            ...
            # write_lower_sensor_trim (0301.TBL)
            self._hrt_frame_write.body = "00000201020101"
        elif cmd == "84":
